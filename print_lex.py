...
if args.query == None:
    for i in range(0, len(lx.lexicon["terms"])):
        if args.template != None:
            fl = fl + lx.get_formatted_term(i, "index", temp=args.template[0])
        else:
            fl = fl + lx.get_formatted_term(i, "index")
else:
    if args.method == None or args.method[0].lower() == "term-part":
        ...
        fl = lx.get_formatted_term(lx.find_term(args.query[0], "term"))
    elif args.method[0].lower() == "index":
        fl = lx.format_term(lx.find_term(int(args.query[0])))
    else:
